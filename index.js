...
const { port, myChatId, telegramBotToken, oa_clientId, oa_clientSecret, oa_redirectUri, oa_accessType, refreshtime } = require('dotenv').config().parsed
...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        res.send('Authentication successful. You can now monitor your Gmail account.');
        console.log("Authentication Successful")
    } catch (error) {
        console.error('Authentication error:', error);
        res.status(500).send('Authentication error. Check the console for details.');
    }
});
